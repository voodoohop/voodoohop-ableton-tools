...
let log = logger('renderContainer');
...
let BeatClickGrid = component2(({ startMarker, endMarker, trackId }) => {
...
export default component2(({ uiState, trackId, track }) => {
  ...
  let viewboxHeight = trackId == 'selectedClip' ? 150 : 200;
  let visibleBeats = uiState.get('visibleBeats');
	//  console.log("children");

  let playingPosX = liveData.get('playingPosition') || 0;

  let endMarker = parseFloat(liveData.get('looping') ? liveData.get('end_marker') : liveData.get('loop_end'));
