...
const log = logger('renderContainer');
...
const BeatClickGrid = component2(({ startMarker, endMarker, trackId }) => {
  ...
  const strokeDasharray = (width, height) => [
  ...
  return (
    <g style={{
      ...
      const strokeWidth = sWidth < 0.05
        ? 0
        : 0.3 + sWidth;
      return <rect
        onClick={(e, f) => beatClick(i * 4, trackId, e, f)}
        className="hoverWhite"
        key={"_beatclickgrid_" + xs[0]}
        fill={`rgba(0,0,0,${i % 16 >= 8
          ? 0.1
          : 0.06}`}
        style={{
          strokeDasharray: strokeDasharray(width, height)
        }}
...
export default component2(({ uiState, trackId, track }) => {
  ...
  let viewboxHeight = trackId == 'selectedClip'
    ? 150
    : 200;
  let visibleBeats = uiState.get('visibleBeats');
  //  console.log("children");

  let playingPosX = liveData.get('playingPosition') || 0;

  let endMarker = parseFloat(liveData.get('looping')
    ? liveData.get('end_marker')
    : liveData.get('loop_end'));
  let startMarker = parseFloat(liveData.get('start_marker'));
  let startRenderPos = Math.min(startMarker, 0);
  let scale = viewboxWidth / visibleBeats; // *
  let shortnessAdjust = 1 / Math.min(1, Math.ceil(4 * (endMarker - startMarker) / 256) / 4);
  if (shortnessAdjust == 3)
    shortnessAdjust = 2;

  // console.log("shortnessAdjust",shortnessAdjust);
  log(playingPosX, uiState, liveData, trackId, track);
  let midiData = track.getIn(['midiData', 'notes']);
  let waveform = track.getIn(['fileData', 'waveform']);
  let waveformLPF = track.getIn(['fileData', 'waveformLPF']);
