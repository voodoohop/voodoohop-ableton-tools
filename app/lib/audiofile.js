export default function AudioFileRequest(url, async) {
    this.url = url;
...
AIFFDecoder.prototype.decode = function(data) {
    ...
    while (offset < fileLength) {
        ...
        if (chunk.name == 'COMM') {
            ...
            var channels = [];
            for (var i = 0; i < numberOfChannels; i++) {
                channels.push(new Float32Array(Math.floor(length/8)));
            }
            ...
            for (var i = 0; i < numberOfChannels; i++) {
                ...
                for (var j = 0; j < length; j++) {
                    ...
                    // Scale range to -1 to 1
                    channel[Math.floor(j/8)] = value / range;
                }
            }
    ...
    decoded.sampleRate = sampleRate/8;
    decoded.bitDepth = bitDepth;
    decoded.channels = channels;
    decoded.length = length/8;
    return decoded;
};